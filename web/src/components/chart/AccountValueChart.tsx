...
export default function AccountValueChart() {
  ...
  useEffect(() => {
    cutoffTimeRef.current = Date.now() - 24 * 3600 * 1000;
  }, []);

  // Update cutoff time when switching to 24H range to ensure it's relative to current time
  useEffect(() => {
    if (range === "24H") {
      cutoffTimeRef.current = Date.now() - 24 * 3600 * 1000;
    }
  ...
  const { data, models } = useMemo(() => {
    ...
    // Percent mode normalization - use initial_balance as base (consistent with total PnL calculation)
    if (mode === "%") {
      // Get initial_balance from account totals for each model
      const rows = (totalsData?.accountTotals ?? []) as any[];
      const initialBalanceByModel = new Map<string, { balance: number; timestamp: number }>();
      
      // Find the latest initial_balance for each model
      ...
      // Use initial_balance if available, otherwise fall back to first data point
      const bases: Record<string, number> = {};
      for (const id of ids) {
        const initialBalanceData = initialBalanceByModel.get(id);
        if (initialBalanceData && initialBalanceData.balance > 0) {
          bases[id] = initialBalanceData.balance;
        } else {
          // Fallback: use first data point if initial_balance not available
          if (!percentBaseRef.current[id]) {
          ...
          bases[id] = percentBaseRef.current[id] || 1; // avoid division by zero
        }
      }
      
      // Update cache with actual bases used
      percentBaseRef.current = { ...bases };
      ...
      points = points.map((p) => {
        ...
        for (const id of ids) {
          ...
          if (typeof v === "number" && base && base > 0) {
            cp[id] = (v / base - 1) * 100; // percent relative to initial_balance
          }
        }
        return cp;
      });
    ...
    // Downsample for 24H range to keep chart performant
  ...
  }, [dataRows, ids, range, mode, totalsData]);
  ...
  return (
    <div
      className={`flex h-full min-h-[260px] sm:min-h-[300px] flex-col rounded-md border p-3 min-w-0`}
      style={{
        ...
        {isLoading ? (
          ...
          <>
            <div className="min-h-0 flex-1">
              <div
                ref={chartRef}
                className="chart-container relative h-full w-full min-h-0 min-w-0 no-tap-highlight select-none"
                tabIndex={-1}
                onMouseDown={(e) => {
