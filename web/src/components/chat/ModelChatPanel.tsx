...
export default function ModelChatPanel() {
  ...
  const list = useMemo(() => {
    ...
    for (const it of items) {
      ...
      const key = `${id}::${ts}::${content}\u0001${user_prompt}`;
    ...
    const filtered = qModel === "ALL" ? arr : arr.filter((r) => r.model_id === qModel);
    return filtered;
  }, [items, qModel]);
  ...
  const options = useMemo(() => {
    ...
    const opts = traders
      .map((t) => {
        ...
        return {
          id,
          label: t.model_name || getModelName(id),
        };
      })
      .filter(Boolean) as { id: string; label: string }[];

    for (const conv of items) {
      const id = String((conv as any)?.model_id || "");
      if (!id || seen.has(id)) continue;
      seen.add(id);
      opts.push({ id, label: getModelName(id) });
    }
  ...
  return (
    <div className="space-y-3">
      <FilterBar model={qModel} onChange={(v) => setModel(v)} options={options} />
      {latestPerModel(filtered).map((row, idx) => (
        <ChatCard
    ...
    <div>
      ...
      <div className="relative pl-8">
        ...
        >
          {title ? (
            <div
              className="ui-sans text-[12px] font-semibold mb-2"
...
function extractTitleAndBody(markdown?: string) {
  ...
  const lines = markdown
    ...
    .map((l) => l.trim())
    .filter(Boolean);
  if (!lines.length) return { title: "", body: markdown };
  const raw = lines[0];
  const title = raw.replace(/^\*+/, "").replace(/\*+$/, "").trim();
  const body = markdown.replace(raw, "").trimStart();
  if (!title) return { title: "", body: markdown };
  return { title, body: body || "" };
}

function latestPerModel(rows: Array<{ model_id: string } & Record<string, any>>) {
