...
export interface AccountTotalsRow {
  ...
  initial_balance?: number; // 初始余额，用于百分比计算
}
...
export function useAccountTotals() {
  ...
  const {
    ...
    ...activityAwareRefresh(10_000, { hiddenInterval: 30_000 }),
  });

  // 添加10秒强制刷新机制（用于持仓页面）
  // 确保即使在有lastMarker的情况下，也能定期刷新以更新动态数据
  useEffect(() => {
    if (typeof window === "undefined") return;
    
    const REFRESH_INTERVAL = 10_000; // 10秒
    let cancelled = false;
