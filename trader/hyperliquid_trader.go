...
func (t *HyperliquidTrader) GetPositions() ([]map[string]interface{}, error) {
	...
	for _, assetPos := range accountState.AssetPositions {
		...
		// Hyperliquid默认使用逐仓模式（Isolated）
		posMap["marginType"] = "isolated"
...
func (t *HyperliquidTrader) GetOpenOrders(symbol string) ([]map[string]interface{}, error) {
	...
	for _, order := range openOrders {
		...
		if err := json.Unmarshal(orderJSON, &orderData); err != nil {
			continue
		}

		// 尝试从订单数据中提取止盈止损信息
		// Hyperliquid的订单结构需要通过JSON解析来访问
		orderType, ok := orderData["orderType"].(map[string]interface{})
		if !ok {
			continue
		}

		trigger, ok := orderType["trigger"].(map[string]interface{})
		if !ok {
			continue // 跳过非触发订单
		}

		tpsl, _ := trigger["tpsl"].(string)
		if tpsl != "sl" && tpsl != "tp" {
			continue
		}

		triggerPxStr, _ := trigger["triggerPx"].(string)
		stopPrice, _ := strconv.ParseFloat(triggerPxStr, 64)

		sz, _ := orderData["sz"].(float64)
		quantity := sz
...
// GetTradeHistory 获取历史成交记录（从Hyperliquid API获取）
// 注意：Hyperliquid API可能不支持直接获取历史成交，暂时返回空数组
func (t *HyperliquidTrader) GetTradeHistory(limit int) ([]map[string]interface{}, error) {
	// TODO: 实现Hyperliquid历史成交获取
	// 目前Hyperliquid API可能不支持此功能，返回空数组
