...
func (tm *TraderManager) AddTrader(cfg config.TraderConfig, coinPoolURL string, maxDailyLoss, maxDrawdown float64, stopTradingMinutes int, leverage config.LeverageConfig) error {
	...
	// 构建AutoTraderConfig
	var streamOpts trader.BinanceStreamOptions
	if cfg.Exchange == "binance" {
		streamOpts.MarketEnabled = cfg.BinanceStream.MarketEnabled
		streamOpts.UserEnabled = cfg.BinanceStream.UserEnabled

		if d := cfg.BinanceStream.MarketReconnectDelay; d != "" {
			if parsed, err := time.ParseDuration(d); err == nil {
				streamOpts.MarketReconnectDelay = parsed
			} else {
				log.Printf("⚠️  无法解析 market_reconnect_delay '%s': %v", d, err)
			}
		}
		if d := cfg.BinanceStream.MarketPingInterval; d != "" {
			if parsed, err := time.ParseDuration(d); err == nil {
				streamOpts.MarketPingInterval = parsed
			} else {
				log.Printf("⚠️  无法解析 market_ping_interval '%s': %v", d, err)
			}
		}
		if d := cfg.BinanceStream.UserReconnectDelay; d != "" {
			if parsed, err := time.ParseDuration(d); err == nil {
				streamOpts.UserReconnectDelay = parsed
			} else {
				log.Printf("⚠️  无法解析 user_reconnect_delay '%s': %v", d, err)
			}
		}
		if d := cfg.BinanceStream.UserPingInterval; d != "" {
			if parsed, err := time.ParseDuration(d); err == nil {
				streamOpts.UserPingInterval = parsed
			} else {
				log.Printf("⚠️  无法解析 user_ping_interval '%s': %v", d, err)
			}
		}
		if d := cfg.BinanceStream.ListenKeyRefreshInterval; d != "" {
			if parsed, err := time.ParseDuration(d); err == nil {
				streamOpts.ListenKeyRefreshInterval = parsed
			} else {
				log.Printf("⚠️  无法解析 listen_key_refresh_interval '%s': %v", d, err)
			}
		}
	}
	...
	traderConfig := trader.AutoTraderConfig{
		...
		BinanceStream:         streamOpts,
		QuantConfig:           cfg.Quant.Copy(),
	}
	...
	log.Printf("✓ Trader '%s' (%s) 已添加", cfg.Name, cfg.Strategy)
	return nil
}
